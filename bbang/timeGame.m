...
@implementation timeGame
@synthesize scoreLabel, score, touchBeganTime;

+(CCScene *) scene
{
	CCScene *scene = [CCScene node];
    timeGame *layer = [timeGame node];
...
-(id)init{
    if (self=[super init]) {
        ...
        scoreLabel = [[CCLabelTTF alloc] initWithString:@" : 0" fontName:@"Arial" fontSize:22];
        ...
        t = MAX_TIME;  //60초
        time = [CCLabelTTF labelWithString:@"60" fontName:@"Arial Black" fontSize:20];
        time.position = ccp(300, 100);
        time.color = ccBLACK;
        //[self addChild:time];
        CCSprite *energyBarBase = [CCSprite spriteWithFile:@"bar_outline1.png"];
        energyBarBase.position=ccp(20, 285);
        energyBarBase.anchorPoint = ccp(0, 0);
        [self addChild:energyBarBase z:21];
        energyM = [CCSprite spriteWithFile:@"bar_inline1.png"];
        [self makeBar];
        ...
        [self createCloudWithSize:FRONT_CLOUD_SIZE top:FRONT_CLOUD_TOP fileName:@"layer_cloud.png" interval:15 z:2];
...
{
	// audio engine
	// preloadEffect를 통해서 Shooting 음과 Scream 음을 생성한다
    ...
    [sound preloadEffect:@"reroad.wav"];
...
-(void)startG{
    ...
    [self schedule:@selector(gameLogic:) interval:a];
}
-(void)comboL{
    CGSize winsize = [[CCDirector sharedDirector] winSize];
...
-(void) Time {
    ...
    if(t == 0) {
        sound3.backgroundMusicVolume = 0;
        [self overA];
    }
}
-(void)makeBar{
    timeBar = [CCProgressTimer progressWithSprite:energyM];
    timeBar.type = kCCProgressTimerTypeBar;
    timeBar.midpoint = ccp(0, .5);//우->좌로만 줄어듬
    timeBar.position = ccp(20, 285);
    timeBar.barChangeRate = ccp(1, 0);//세로 변동x, 가로만 변함
    timeBar.anchorPoint = ccp(0, 0);
    timeBar.percentage=100;
    [self addChild:timeBar z:21];
}
-(void)timer{
    if (t<0) t=0;
	float timeRate =(float)t/60;
...
-(void)gameLogic:(ccTime)dt{
    timeM = arc4random()%3+1;
    [self schedule:@selector(gameLogic:) interval:timeM];
    if (t > 45 && t<=60) {
        [self makeTarget];
    }
    else if (t > 30 && t <= 45){
...
-(void)makeTarget {
    ...
    NSInteger startW = -80;
    NSInteger startH = (arc4random()%180)+50;
    ...
    for (CCSprite *gold in [goldNode children]) {
        if (CGRectContainsPoint([gold boundingBox], convertedLocation)) {
            ...
            if (bulletNum<=10) {
                ...
                if (combo>=2) {
                    delegate.gameScore += 30*combo;
                    NSString *str = [[NSString alloc] initWithFormat:@": %d", delegate.gameScore];
                    [scoreLabel setString:str];
                    [comboMess showMessage:COMBO_MESSAGE];
                    [self.scoreNumSprite setString:[NSString stringWithFormat:@"%d", combo]];
                    [self.scoreNumSprite runAction:[CCFadeTo actionWithDuration:0.01 opacity:0]];
                    [self.scoreNumSprite runAction:[CCSequence actions:
                                                    [CCFadeTo actionWithDuration:0.1 opacity:250],
                                                    [CCDelayTime actionWithDuration:0.2],
